...
Each token is separated by white space or some other recognizably different token, like an operator, for example, = or +.
...
`global` - keyword allows all imports of a namespace to one `.cs` file making the namespace available to the entire project.
	Regular `using` only applies to the single file it sits in.
	`global using` applies to every file in the compilation.

Defining imports in `.csproj` file makes them generated in the `.GlobalUsings.g.cs` file generated by the compiler.

`using` - whether local or global is purely a compile-time name-resolution convenience.
	It produced zero IL.
	At runtime there is no such thing as an imported namespace.

Choose global vs local on readability and scope-hygiene grounds, not performance.
